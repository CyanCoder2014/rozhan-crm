...
namespace Modules\TicketingModule\Http\Controllers;
 ...
 * @package Modules\TicketingModule\Http\Controllers
 *
 * For Outer User
 */
class TicketController extends Controller
{
    ...
    {
        $tickets = Ticket::select('tickets.*')->where(function ($query) {
                $query->where('answerable_id', auth()->id())
        ...
        return view('ticketingmodule::ticket.index',compact('tickets','categories'));
    }
    ...
    {
        if (!$this->reletadToTicket(auth()->user(),$ticket))
            return abort(404);
        return view('ticketingmodule::ticket.show',compact('ticket'));
    }
    ...
    public function reletadToTicket(User $user, Ticket $ticket): bool {
        if ($user->id == $ticket->owner_id || $user->id == $ticket->answerable_id)
            return true;
        return false;
    }
}
