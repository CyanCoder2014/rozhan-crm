...
class ProfileController extends BaseAPIController
{
    ...
    {
        return UserProfile::select('id','first_name', 'last_name')->get();
    }

    public function index()
    {
        $columns =['contact_id','age','major','education_field','work_field','national_code',
            'gender','birth','about','visitor_note','type','state',
            'regent','reagent_detail','reagent_state','reagent_page','reagent_site','reagent_type','regent_id',
            'weight','height',
            'disease','drug','heart','respiratory','Liver','Diabetes','Seizure','infectiousdisease','tumor',
            'asthma','hormonaldisorder','otherHerpes','asprin','deyper','convulsion','arpharin',
            'heparin','minoxide','rakuten','cream','otherpharmacy',
            'boronz','surgery','typeSurgery','getpregnant','milch','Botox','zhel','Kidney','thyroid',
            'hormonal','allergy','Laser',
            'text'];
        $search_column=['contact_id','age','major','education_field','work_field','national_code',
            'gender','birth','about','visitor_note','type','state',
            'regent','reagent_detail','reagent_state','reagent_page','reagent_site','reagent_type','regent_id',
            'weight','height',
            'disease','drug','heart','respiratory','Liver','Diabetes','Seizure','infectiousdisease','tumor',
            ...
            'hormonal','allergy','Laser',
            'text',];
        $with=['contact.user','contact'];
        if (!$search_column)
            $search_column = $columns;
        if ( \request()->input('showdata') ) {
            return UserProfile::orderBy('created_at', 'desc')->get();
        }
        $length = \request()->input('length',15);
        $column = \request()->input('column');
        $order = \request()->input('order','desc');
        $search_input = \request()->input('search');
        $query = UserProfile::select(array_merge($columns,['id','created_at']))
